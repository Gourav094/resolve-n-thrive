...
export const authApi = {
  login: async (email, password, regToken = null) => {
    try {
      const headers = {};
      // Include registration token in Authorization header if available
      if (regToken) {
        headers.Authorization = `Bearer ${regToken}`;
      }
      
      const response = await api.post('/api/auth/login', { email, password }, { headers });
...
export const grievanceApi = {
  ...
  filterGrievances: async (filters) => {
